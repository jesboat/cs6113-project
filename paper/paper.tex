...
\begin{description}
  ...
  \item[Correction of the Soundness Lemma]
    ...
    formulation in Simonet and Pottier's paper on \origLang~\cite{InfoFlowML}
    wasn't exactly correct as stated.  Specifically, it is entirely possible
    that a step in the operational semantics does not translate to a step in the
    operational semantics under either projection, as in the case of the
    \ref{eq:lift-app} rule, where $\lift{e}_i$ and $\lift{e'}_i$ are identical.
    Luckily, we are fairly confident, given that we were still able to prove
